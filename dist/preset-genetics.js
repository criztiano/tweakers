...
function seedDNA(baseline, parameters, settings, random = Math.random) {
  const result = cloneDNA(baseline);
  for (const p of parameters.filter((p2) => p2.enabled)) {
    ...
    else if (settings.spread > 0) write(result, p, p.kind === "number" ? numeric(current + (random() * 2 - 1) * settings.spread * ((p.high ?? p.max ?? 1) - (p.low ?? p.min ?? 0)), p) : random() < settings.spread ? randomGene(p, random) : current);
  }
  return repairRanges(result, parameters);
}
function breedDNA(a, b, baseline, parameters, settings, random = Math.random) {
...
//# sourceMappingURL=preset-genetics.js.map