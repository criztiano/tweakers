...
var TweakStoreClass = class {
  ...
  updatePanel(id, name, config, shortcuts, options = {}) {
    ...
    const nextPanel = { id, name, controls, values: nextValues, shortcuts: shortcuts ?? existing.shortcuts, hints, affordances, labels, movePads, moveTopRow, moveActionRow, moveValueRow, moveSlotGroups, moveBands, module: "_enabled" in config ? true : void 0, kind: options.kind ?? existing.kind };
  ...
  overlayPersistedValues(panelId, target, values, controlsByPath) {
    ...
    for (const key of Object.keys(persisted)) {
      ...
      if (restored === void 0) dropped.push(key);
   ...
   */
  reconcileValue(incoming, defaultValue, path, controlsByPath) {
    if (path.endsWith(".__mode")) {
      const owner = controlsByPath.get(path.slice(0, -".__mode".length));
      return owner?.type === "transition" && (incoming === "easing" || incoming === "simple" || incoming === "advanced") ? incoming : void 0;
    }
   ...
   */
  reconcileSnapshot(panel, incoming) {
    ...
    for (const [path, value] of Object.entries(incoming)) {
      ...
      if (restored !== void 0) next[path] = restored;
    }
    return next;
  }
  presetSchema(c) {
  ...
  isToggleConfig(value) {
    return typeof value === "object" && value !== null && "type" in value && value.type === "toggle";
  }
