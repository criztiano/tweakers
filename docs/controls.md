...
whole wheel back. What the app writes itself stays the app's to keep on the
palette.
...
| `MoveColorStore.lockPalette` | An app-wide palette: every colour control held to its colours, open editor or not, on screen and on the hardware. `null` releases it. See the colour section above. |
