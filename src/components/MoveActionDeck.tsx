...
function DeckButton({ action }: { action: MoveDeckAction }) {
  ...
  return (
    ...
    >
      <MoveFunctionGlyphIcon glyph={MOVE_FUNCTION_ICONS[action.button]} className="tweakers-move-deck-icon" />
...
export function MoveActionDeck({ actions, className }: MoveActionDeckProps) {
  const { actions: shown, warnings } = useMemo(() => normalizeDeck(actions), [actions]);
