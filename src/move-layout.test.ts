...
import { buildMovePages, normalizeDial, MOVE_TRACKS, MOVE_DIALS, MOVE_PADS } from './move-layout';

// The MovePanel mirrors the bridge kit's v0 mapping: first 4 panels are the
// track pages, sliders and bounded numbers fill the 8 dials, toggles the 8
// pads. These tests pin that contract on the library side.
...
describe('move layout', () => {
  it('maps sliders and bounded numbers to dials, toggles to pads', () => {
    ...
    assert.deepEqual(page.pads.map((p) => p.path), ['mute', 'nested.wet']);
  });

  it('caps pages at 4 tracks and slots at 8 dials / 8 pads', () => {
    ...
    assert.equal(pages[0].pads.length, MOVE_PADS);
  });
