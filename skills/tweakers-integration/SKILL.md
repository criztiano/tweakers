...
| A color that is a mix of two | `balance` (`{ type: 'balance', a, b }`) plus its two `color` params — declare the three params and STOP: the kit seats the two colors itself, stacked in the balance's own column, lit in their colors, each a chip (tap latches it onto the balance's knob, hold peeks). No `movePads`, no wiring |
...
own column (lit in their colors, chips like any other — declaring the three params is
