...
describe('the type registry', () => {
  it('ships the LFO, the S&H, the ADSR, the curve and the audio, in that order', () => {
    ...
    expect(getModType('audio')).toBe(AUDIO_DEF);
    expect(listModTypes().map((d) => d.type)).toEqual(['lfo', 'sh', 'adsr', 'curve', 'audio']);
  });
